...
def drums_data_generator(model_params=None):
    ...
    if not masking==0:
        if masking==1:
            fig_name = fig_name + '_maskingthresh'
        elif masking==2:
            fig_name = fig_name + '_maskinguni'
        elif masking==3:
            fig_name = fig_name + '_maskingnorm'
    ...
    sim.plot(to_plot=['new_infections_by_variant','new_infections', 'new_tests', 'new_diagnoses', 'cum_diagnoses', 'new_quarantined', 'test_yield'],
             do_show=False)
    plt.savefig('../../Notebooks/figs/drums/tests/' + fig_name + '.png', dpi=300)
    plt.close()
